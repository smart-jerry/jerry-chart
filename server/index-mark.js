...
router.post('/getTotal',(req, res) => {
  ...
  config.db.collection(reqb.regionName).find({}).count(function(err, result) {
    if (err) throw err;
...
//  console.log(req.body,'22222222222222222222222222');
  let reqb = req.body;
  config.db.collection(reqb.regionName). find({}).skip(reqb.skip || 0).limit(reqb.limitNum || 10).toArray(function(err, result) { // 返回集合中所有数据
    if (err) throw err;
//    console.log(result);
    res.send(result)
  });
